...
import type { DiagnosticsSnapshot } from "@instafix/core";
...
describe("Diagnostics", () => {
  it("merges console + network entries in chronological order", () => {
    ...
    const entries = [...container.querySelectorAll(".ifd-diag-entry")];
  ...
  it("renders the entries as a semantic list of <li> items", () => {
    const { container } = renderWithUi(<Diagnostics diagnostics={makeDiagnostics()} />);
    expect(container.querySelector("ul.ifd-diag-list")).not.toBeNull();
    expect(container.querySelectorAll("li.ifd-diag-entry")).toHaveLength(3);
  });

  it("renders timestamps as <time> with a machine-readable dateTime", () => {
    const { container } = renderWithUi(<Diagnostics diagnostics={makeDiagnostics()} />);
    const time = container.querySelector<HTMLTimeElement>("time.ifd-diag-time");
  ...
  it("flags failed network entries (status >= 400)", () => {
    const { container } = renderWithUi(<Diagnostics diagnostics={makeDiagnostics()} />);
    const failed = container.querySelector<HTMLElement>('.ifd-diag-entry[data-level="error"] .ifd-diag-status');
  ...
  it("marks a 2xx network entry as info, not failed", () => {
    ...
    expect(container.querySelector('.ifd-diag-entry[data-level="info"]')).not.toBeNull();
    expect(container.querySelector<HTMLElement>(".ifd-diag-status")?.dataset.failed).toBeUndefined();
  });
  ...
  it("collapses to the first six entries with a Show all button, then reveals the rest", () => {
    ...
    const { container } = renderWithUi(<Diagnostics diagnostics={diagnostics} />);
    expect(container.querySelectorAll(".ifd-diag-entry")).toHaveLength(6);
    const showAll = screen.getByRole("button", { name: "Show all (8)" });
    fireEvent.click(showAll);
    expect(container.querySelectorAll(".ifd-diag-entry")).toHaveLength(8);
  });
  ...
  it("renders a real <button> expander for a clampable message and toggles it", () => {
    ...
    const entry = button.closest(".ifd-diag-entry") as HTMLElement;
  ...
  it("renders a short message as plain text, not an expander button", () => {
    ...
    const msg = container.querySelector<HTMLElement>(".ifd-diag-msg");
