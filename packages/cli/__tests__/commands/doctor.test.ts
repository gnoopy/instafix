...
describe("doctorCommand", () => {
  ...
  describe("fetch success", () => {
    it("shows success message when response has data.total", async () => {
      ...
      await doctorCommand({ url: "http://localhost:3000", endpoint: "/api/instafix" });

      expect(p.intro).toHaveBeenCalledWith("instafix — Network diagnostics");
      expect(spinnerMock.start).toHaveBeenCalledWith("Testing connection to http://localhost:3000/api/instafix");
  ...
  describe("HTTP errors", () => {
    it("shows error + body text for HTTP error with body", async () => {
      ...
      const err = await doctorCommand({ url: "http://localhost:3000", endpoint: "/api/instafix" }).catch((e) => e);
  ...
  describe("network errors", () => {
    ...
    it("shows connectivity message for TypeError containing 'fetch'", async () => {
      ...
      const err = await doctorCommand({ url: "http://localhost:9999", endpoint: "/api/instafix" }).catch((e) => e);
  ...
  describe("URL validation", () => {
    it("exits(1) for invalid URL without http(s) prefix", async () => {
      const err = await doctorCommand({ url: "ftp://example.com", endpoint: "/api/instafix" }).catch((e) => e);
  ...
  describe("URL construction", () => {
    it("builds correct URL with encoded project name", async () => {
      ...
      expect(calledUrl).toBe("http://localhost:3000/api/instafix?projectName=__instafix_health_check__");
    });
  });
  ...
  describe("spinner", () => {
    ...
    it("stops spinner with 'Connection failed' on network error", async () => {
      ...
      await doctorCommand({ url: "http://localhost:3000", endpoint: "/api/instafix" }).catch(() => {});
