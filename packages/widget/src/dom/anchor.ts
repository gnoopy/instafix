...
/** An ancestor whose box would cover more than this fraction of the viewport reads as a page-wide layout wrapper, not a "component". */
const LARGEST_ANCESTOR_AREA_CAP = 0.6;

/**
 * Climb from `el` toward the document root, returning the outermost ancestor
 * that's still a reasonably-scoped "component" — the counterpart to
 * `findAnchorElement`'s deepest/most-specific pick, used to offer a right-click
 * "smallest vs. largest" choice (G8) when the two genuinely differ. Stops
 * before `<body>`/`<html>`, widget chrome, or the first ancestor whose box
 * would exceed `LARGEST_ANCESTOR_AREA_CAP` of the viewport — a page section
 * or layout wrapper is never "the component" the user is pointing at.
 */
export function findLargestAncestor(el: Element): Element {
  const viewportArea = window.innerWidth * window.innerHeight;
  let current = el;
  for (;;) {
    const parent = current.parentElement;
    if (!parent || parent === document.body || parent === document.documentElement) break;
    if (isWidgetChrome(parent)) break;
    const box = parent.getBoundingClientRect();
    if (box.width <= 0 || box.height <= 0) break;
    if (box.width * box.height > viewportArea * LARGEST_ANCESTOR_AREA_CAP) break;
    current = parent;
  }
  return current;
}
