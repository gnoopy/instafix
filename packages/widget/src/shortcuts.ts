...
export class KeyboardShortcuts {
  ...
  private handleKeydown(e: KeyboardEvent): void {
    ...
    // If help overlay is open, trap Tab inside it and block all other shortcuts
    if (this.helpVisible) {
      if (e.key === "Tab") this.trapHelpFocus(e);
      return;
    }
  ...
  /** Keep Tab focus cycling within the help overlay while it's open. */
  private trapHelpFocus(e: KeyboardEvent): void {
    const focusable = Array.from(
      this.helpOverlay.querySelectorAll<HTMLElement>(
    ...
    const root = this.helpOverlay.getRootNode() as Document | ShadowRoot;
