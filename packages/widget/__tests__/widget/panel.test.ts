...
/** Confirm a pending G7 delete confirm dialog (single/bulk/detail — same `.sp-btn-danger` pattern as deleteAll). */
async function confirmDeleteDialog(shadow: ShadowRoot): Promise<void> {
  await vi.waitFor(() => {
    expect(shadow.querySelector(".sp-btn-danger")).not.toBeNull();
  });
  shadow.querySelector<HTMLButtonElement>(".sp-btn-danger")!.click();
}
...
describe("Panel", () => {
  ...
  describe("construction", () => {
    ...
    it("keeps the close button on its own row with only the title (never sharing a row with the action toolbar)", () => {
      // Regression test: the close button must never be able to be pushed out
      // of the visible panel by an overflowing action-button row — it lives
      // in a row with exactly the title, which is the only thing that can
      // compete with it for space.
      const headerTop = shadow.querySelector<HTMLElement>(".sp-panel-header-top")!;
      expect(headerTop).not.toBeNull();
      expect(headerTop.querySelector(".sp-panel-title")).not.toBeNull();
      expect(headerTop.querySelector(".sp-panel-close")).not.toBeNull();

      const headerActions = shadow.querySelector<HTMLElement>(".sp-panel-header-actions")!;
      expect(headerActions).not.toBeNull();
      // The close button must not live in the (unbounded-growth) actions row.
      expect(headerActions.querySelector(".sp-panel-close")).toBeNull();
      expect(headerActions.querySelector(".sp-btn-delete-all")).not.toBeNull();
    });
  });
  ...
  describe("open/close", () => {
    ...
    describe("click outside closes the panel", () => {
      it("closes when a click lands outside the panel entirely", async () => {
        await panel.open();
      ...
      it("stays open when a click lands on a card inside the panel", async () => {
      ...
      it("stays open when a click lands on the panel's own header", async () => {
        await panel.open();

        const title = shadow.querySelector<HTMLElement>(".sp-panel-title")!;
      ...
      it("does nothing when the panel is already closed", () => {
        const outside = document.createElement("button");
        document.body.appendChild(outside);
        // Should not throw when closed — no listener side effects to assert
        // beyond "still closed" and "no error".
        expect(() => outside.dispatchEvent(new MouseEvent("click", { bubbles: true, composed: true }))).not.toThrow();
        const root = shadow.querySelector<HTMLElement>('[role="complementary"]')!;
        expect(root.getAttribute("aria-hidden")).toBe("true");
        outside.remove();
      });

      it("removes the outside-click listener on destroy", async () => {
        await panel.open();
        panel.destroy();

        const outside = document.createElement("button");
        document.body.appendChild(outside);
        // Should not throw after destroy (this.root has been removed from the DOM).
        expect(() => outside.dispatchEvent(new MouseEvent("click", { bubbles: true, composed: true }))).not.toThrow();
        outside.remove();
      });
    });
  ...
  describe("card actions", () => {
    ...
    it("shows a distinct confirm dialog for bulk delete naming the selected count", async () => {
      ...
      await vi.waitFor(() => {
        const dialog = shadow.querySelector<HTMLElement>(".sp-confirm-dialog");
  ...
  describe("detail view callbacks", () => {
    ...
    it("editing the message calls client.updateFeedbackMessage, reloads, and re-shows the detail view with the new text", async () => {
      const fb = makeFeedback({ id: "fb-1", status: "open", message: "original text", annotations: [annotation] });
      const updated = makeFeedback({ id: "fb-1", status: "open", message: "edited text", annotations: [annotation] });
      apiClient.getFeedbacks.mockResolvedValueOnce({ feedbacks: [fb], total: 1 });
      apiClient.updateFeedbackMessage.mockResolvedValue(updated);
      apiClient.getFeedbacks.mockResolvedValue({ feedbacks: [updated], total: 1 });

      await panel.open();
      shadow.querySelector<HTMLElement>('[data-feedback-id="fb-1"]')!.click();

      const editBtn = shadow.querySelector<HTMLButtonElement>(".sp-detail-message-edit-btn")!;
      expect(editBtn).not.toBeNull();
      editBtn.click();

      const textarea = shadow.querySelector<HTMLTextAreaElement>(".sp-detail-message-textarea")!;
      expect(textarea.value).toBe("original text");
      textarea.value = "edited text";

      const saveBtn = shadow.querySelector<HTMLButtonElement>(".sp-detail-btn-save")!;
      saveBtn.click();

      await vi.waitFor(() => {
        expect(apiClient.updateFeedbackMessage).toHaveBeenCalledWith("fb-1", "open", "edited text");
      });

      await vi.waitFor(() => {
        const detail = shadow.querySelector<HTMLElement>(".sp-detail")!;
        expect(detail.classList.contains("sp-detail--visible")).toBe(true);
        expect(detail.textContent).toContain("edited text");
        // Edit mode implicitly ends on the re-render — the edit button is back.
        expect(shadow.querySelector(".sp-detail-message-edit-btn")).not.toBeNull();
      });
    });

    it("cancelling a message edit discards the draft and restores the read-only view", async () => {
      const fb = makeFeedback({ id: "fb-1", message: "original text", annotations: [annotation] });
      apiClient.getFeedbacks.mockResolvedValue({ feedbacks: [fb], total: 1 });

      await panel.open();
      shadow.querySelector<HTMLElement>('[data-feedback-id="fb-1"]')!.click();
      shadow.querySelector<HTMLButtonElement>(".sp-detail-message-edit-btn")!.click();

      const textarea = shadow.querySelector<HTMLTextAreaElement>(".sp-detail-message-textarea")!;
      textarea.value = "unsaved draft";

      const cancelBtn = Array.from(
        shadow.querySelectorAll<HTMLButtonElement>(".sp-detail-message-actions button"),
      ).find((btn) => btn !== shadow.querySelector(".sp-detail-btn-save"))!;
      cancelBtn.click();

      expect(apiClient.updateFeedbackMessage).not.toHaveBeenCalled();
      const detail = shadow.querySelector<HTMLElement>(".sp-detail")!;
      expect(detail.textContent).toContain("original text");
      expect(shadow.querySelector(".sp-detail-message-edit-btn")).not.toBeNull();
    });

    it("keeps the user's edited text in the textarea when saving fails", async () => {
      const fb = makeFeedback({ id: "fb-1", message: "original text", annotations: [annotation] });
      apiClient.getFeedbacks.mockResolvedValue({ feedbacks: [fb], total: 1 });
      apiClient.updateFeedbackMessage.mockRejectedValue(new Error("network down"));
      ...
      const textarea = shadow.querySelector<HTMLTextAreaElement>(".sp-detail-message-textarea")!;
      textarea.value = "my careful edit";
      shadow.querySelector<HTMLButtonElement>(".sp-detail-btn-save")!.click();

      await vi.waitFor(() => {
        expect(errorListener).toHaveBeenCalledWith(expect.any(Error));
      });

      // Same textarea, same value — nothing was lost or reset on failure.
      const textareaAfter = shadow.querySelector<HTMLTextAreaElement>(".sp-detail-message-textarea")!;
      expect(textareaAfter).toBe(textarea);
      expect(textareaAfter.value).toBe("my careful edit");
