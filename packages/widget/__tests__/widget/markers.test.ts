...
describe("MarkerManager", () => {
  ...
  // -------------------------------------------------------------------------
  // Area-kind targets (G4) — no DOM element, positioned from absolute
  // page coordinates instead of going through the element resolver.
  // -------------------------------------------------------------------------

  describe("area-kind target geometry", () => {
    it("positions the marker from scroll + percentage-of-capture-viewport, bypassing the resolver", () => {
      ...
      // The mocked resolveAnnotation must never be invoked for an area target.
      expect(mockState.calls).toBe(callsBefore);

      const marker = document.querySelector<HTMLElement>("[data-feedback-id]")!;
      // top-left of the region = scrollY + yPct*viewportH, scrollX + xPct*viewportW;
      // marker anchors to the top-right corner minus the 13px offset (window.scrollY/X are 0 in this test).
      const expectedTop = 200 + 0.5 * 800 - 13;
      const expectedRight = 0 + 0.25 * 1000 + 0.1 * 1000;
      expect(marker.style.top).toBe(`${expectedTop}px`);
      expect(marker.style.left).toBe(`${expectedRight - 13}px`);
    });

    it("always renders solid/full-opacity (no ambiguity to flag) regardless of the mocked confidence", () => {
      mockState.confidence = 0.1; // would be low-confidence for an element target
      markers.render([makeFeedback({ annotations: [makeAnnotation({ target: { kind: "area" } })] })]);

      const marker = document.querySelector<HTMLElement>("[data-feedback-id]")!;
      expect(marker.style.borderStyle).toBe("solid");
      expect(marker.style.opacity).not.toBe("0.7");
    });

    it("repositions an area marker on a reposition tick without touching the element resolver", () => {
      vi.useFakeTimers();
      try {
        markers.render([makeFeedback({ annotations: [makeAnnotation({ target: { kind: "area" } })] })]);
        const callsBefore = mockState.calls;
        ...
        expect(mockState.calls).toBe(callsBefore);
        const markerEl = document.querySelector<HTMLElement>("[data-feedback-id]")!;
