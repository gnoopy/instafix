...
Ajoutez `data-instafix-ignore="true"` à tout élément qui ne doit jamais apparaître dans une capture. L'élément et tous ses descendants sont retirés du rendu :
...
La valeur doit être exactement `"true"` — l'attribut nu ou `"false"` ne masque rien. L'interface du widget elle-même s'exclut de la même façon, donc les captures ne contiennent jamais l'habillage InstaFix.

> Masquer un élément désactive aussi le [commentaire au clic droit](/fr/docs/widget/right-click) dessus — le widget traite `data-instafix-ignore` comme un « ne pas y toucher ».
