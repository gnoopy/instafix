...
This is one of three ways to hand a fix note to an agent — the clipboard one, works with any store. For the other two (dispatch via CLI, or pull into a running session) see [Agent workflow](/docs/agent-workflow).
...
## Region numbers and component numbers

Outer badges `#1`, `#2` identify selection regions. Inner numbers `1`, `2`, `3` identify components **only within their own region**. For example: `Remove 1 and expand 2. Match the spacing of #1 component 2.` The prompt explicitly distinguishes `#1` from local component `1`.

Region numbers and captured context are retained per project and page in the current browser tab session. Sorting and filtering do not renumber regions; deleted numbers are not reused. Unsaved selections can be referenced, and submitting a selection preserves its region number. These are not global identifiers shared between users or browser tabs.

Explicit `#N` references automatically include the other region's selectors, component details, bounds and screenshot path as **reference context**, without replaying its old request as another task. Use `#N` to identify a related region; vague prose is not used to guess a match. Missing or foreign-page references are reported as unavailable. Exports without browser region context, such as CLI exports, do not infer region numbers from list order.

With screenshots enabled, images are captured on selection. Stored screenshots use their storage paths. Unsaved or inline images download on copy as files such as `instafix-region-N-identifier.jpg`, and their filenames appear in the prompt. **The clipboard contains text only: attach the downloaded images to your agent when needed.** Disabled or failed captures are explicitly reported as unavailable.

Generated prompt prose follows the widget's Korean or English locale; requests and selectors retain their original text.

## What you get

Paste this straight into your agent's prompt. This is an English excerpt for one feedback with a single element target; numbering and screenshot guidance are omitted:
