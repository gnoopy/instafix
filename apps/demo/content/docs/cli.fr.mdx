...
> **Le nom du package compte.** Lancez toujours `npx @instafix/cli <commande>`. Il n'existe pas de package `instafix` sur npm : `npx instafix` ne fonctionne que si `@instafix/cli` est déjà installé dans votre projet.
...
2. **Générer la route API** — propose de créer une route Next.js App Router qui sert le widget. Le fichier va dans `app/api/instafix/route.ts`, ou `src/app/api/instafix/route.ts` si votre projet utilise une arborescence `src/`. Une route existante n'est jamais écrasée.

La route générée importe deux choses que vous devez fournir vous-même — le CLI n'installe rien :

- le package `@instafix/adapter-prisma` (`npm i @instafix/adapter-prisma`)
