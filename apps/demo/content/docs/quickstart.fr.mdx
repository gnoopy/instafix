...
## Référence : mode sans serveur (local)

Tout ce qui précède suppose un store côté serveur — c'est ce qui alimente la boucle agent : `instafix prompt`, `/instafix` et le bouton « To agent » lisent les fix notes depuis le serveur (par exemple [le dossier `.instafix/`](/fr/docs/adapters/fs)), donc un store qui ne vit que dans le navigateur ne peut pas les servir.

Le mode local existe pour le cas où il n'y a aucun serveur : un site statique, une maquette, un prototype jetable. Les fix notes restent dans le navigateur du visiteur (localStorage) et n'en sortent jamais :
...
Ce site de documentation tourne exactement avec cette configuration — essayez le widget ici même. Options et limites : [adaptateur localStorage](/fr/docs/adapters/localstorage).
