...
Confirmez les deux questions : le CLI ajoute les deux modèles InstaFix à votre schéma Prisma et génère la route API (`app/api/instafix/route.ts`, ou sous `src/app/` selon votre projet). Poussez ensuite le schéma vers votre base :
...
export function Feedback() {
  useInstaFix({ endpoint: "/api/instafix", projectName: "mon-projet" });
  return null;
}
```
