...
2. **Generate the API route** — offers to create a Next.js App Router route that serves the widget. The file goes to `app/api/instafix/route.ts`, or `src/app/api/instafix/route.ts` when your project uses a `src/` layout. An existing route is never overwritten.
