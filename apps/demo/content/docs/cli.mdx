...
The SQLite-flavored route only needs `@instafix/adapter-sqlite` installed (`npm install github:gnoopy/instafix#adapter-sqlite-dist`) — no client to wire up, no schema to push; see the [SQLite adapter](/docs/adapters/sqlite).
