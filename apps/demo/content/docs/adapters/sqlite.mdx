...
`createInstaFixHandler` here is the exact same handler factory `@instafix/adapter-prisma` exports — same auth, CORS, validation, and webhooks — just re-exported so you never need to `npm i` or import anything Prisma-flavored:
