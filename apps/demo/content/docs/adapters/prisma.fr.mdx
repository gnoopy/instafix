...
`@instafix/adapter-prisma` transforme une URL de votre app en API de feedback complète. Il gère Prisma Client v5, v6 et v7, et exige Node 22+.
...
Voici le schéma exact dont l'adapter a besoin — c'est ce que génère `npx github:gnoopy/instafix#cli-dist sync`. Préférez le CLI ; si vous l'écrivez à la main, ne retirez pas de colonnes : l'adapter écrit `urlPattern`, `screenshotUrl` et `anchorKey` à chaque insertion, donc un schéma partiel échoue dès le premier envoi.
