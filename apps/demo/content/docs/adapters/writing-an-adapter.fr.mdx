...
description: Construisez votre propre adapter de stockage avec @instafix/adapter-kit — le moteur, le contrat et la suite de conformité.
---

Les trois adapters fournis couvrent la plupart des installations, mais les stores InstaFix sont enfichables : tout ce qui peut conserver des enregistrements peut alimenter le widget et le dashboard. `@instafix/adapter-kit` est le package publié pour ça — il porte le contrat `InstaFixStore`, les briques sur lesquelles tournent les adapters officiels, et la suite de conformité qui prouve que votre implémentation se comporte comme les autres.

```bash
npm i -D @instafix/adapter-kit vitest
```

`vitest` est une **peer dependency optionnelle**, nécessaire uniquement à l'entrée `@instafix/adapter-kit/testing`. Si vous ne lancez pas la suite de conformité, laissez-la de côté. Le kit lui-même n'a aucune dépendance runtime (`@instafix/core` est intégré dans son `dist`) : une dépendance de développement suffit si vous bundlez votre package ; si vous publiez du code non bundlé, déplacez-la dans `dependencies`.

> Pourquoi un package séparé : `@instafix/core` est un package interne — il exporte du TypeScript brut et n'est jamais publié sur npm. Le kit est la façon supportée de dépendre de ces types et helpers depuis l'extérieur de ce dépôt.
...
| Requêtes — SQL, un ORM, une API HTTP | Les 6 méthodes de `InstaFixStore` | [Backends à requêtes](#backends-à-requêtes) |
...
`createCollectionStore` renvoie un `CollectionStore` — un `InstaFixStore` dont `verifyProjectOwnership` est garanti au lieu d'être optionnel, vous pouvez donc lui déléguer sans vérification préalable.
...
```ts
// Serveur — les handlers de requêtes acceptent n'importe quel InstaFixStore, Prisma ou non
...
initInstaFix({ store: new MyStore(), projectName: "mon-app" });
```

Si vous en publiez un, [ouvrez une issue](https://github.com/gnoopy/instafix/issues) — nous le référencerons depuis cette page.
