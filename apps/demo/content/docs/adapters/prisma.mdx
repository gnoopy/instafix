...
`@instafix/adapter-prisma` turns one URL of your app into a complete feedback API. It supports Prisma Client v5, v6, and v7, and requires Node 22+.
...
|---|---|---|---|
| `prisma` | `InstaFixPrismaClient` | — | Your Prisma client. Required unless you pass `store` |
| `store` | `InstaFixStore` | — | Use any store instead of Prisma (takes precedence) |
| `apiKey` | `string` | — | Enables Bearer auth. Requests send `Authorization: Bearer <key>` |
| `publicEndpoints` | `InstaFixHttpMethod[]` | `["POST", "OPTIONS"]` when `apiKey` is set | Methods that skip auth. **Passing a value replaces the default** — include `"POST"` yourself or the widget can no longer submit |
...
This is the exact schema the adapter needs — it's what `npx github:gnoopy/instafix#cli-dist sync` generates. Prefer the CLI; if you write it by hand, don't drop columns: the adapter writes `urlPattern`, `screenshotUrl`, and `anchorKey` on every insert, so a partial schema fails on the first submission.
...
If an upload fails, the feedback is still saved (with `screenshotUrl: null`) and a warning is logged — a broken bucket never loses a fix note.
...
`createInstaFixHandler({ store })` mounts any `InstaFixStore` (the memory adapter, or your own) behind the same HTTP surface — same validation, auth, and redaction.

Two caveats, straight from the source:

- The cross-project ownership check on PATCH/DELETE is duck-typed on the optional `store.verifyProjectOwnership` method — implement it and the handler enforces it automatically (`PrismaStore`, `MemoryStore`, and `LocalStorageStore` all do). Without it, any caller who knows a feedback `id` can modify it regardless of `projectName` — put such custom-store endpoints behind an `apiKey`.
- `screenshotStorage` and `caseInsensitiveSearch` only apply to the built-in Prisma path; with a custom store, handle screenshots and search casing inside your store.
